...
Directories in the shell have a tree-like structure. You can use `cd ..` to move up one step in the tree, `cd ../..` to move up two steps, `cd ../../..` to move up three steps, and so on.
...
You can see the envioronment variables by doing:
...
You can write shell scripts that take input variables. Create a file:

```sh
nano getonegenus.sh
```

Write the following. Here `$1` represents the input variable:
