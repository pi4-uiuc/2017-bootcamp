...
* data analysis by design
  * formulas for statistical models e.g. `r yield ~ genotype + nitrogen + irrigation` 
  * vectorization, eg. `r a <- 4 * X` instead of `for i in `r 1:length(X) a[i] <- 4 * X[i]`
...
```{r data-dont-exist-error, eval = FALSE}
...
sum(salix_yield$mean > 10, na.rm = TRUE)
