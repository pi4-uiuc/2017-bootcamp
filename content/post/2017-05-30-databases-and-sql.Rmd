...
OpenRefine supports faceted browsing. This allows you to see the big picture of your data and
...
A [.csv file](https://en.wikipedia.org/wiki/Comma-separated_values) is a table where each column is defined by a comma, e.g. a 3x3 matrix would be formatted thus:
...
To motivate the use of a relational databases, lets start by looking at a table. Is there redundant information that violates the best practice of having a single canonical representation for each piece of information?

> YOUR TURN: which of these fields has redundant data? 
...
^^ from [Data Carpentry](http://www.datacarpentry.org/sql-ecology-lesson/00-sql-introduction/) 
...
SQL Connection parameters include:
...
Open the PostgreSQL Studio application in the NDSLabs workbench and enter the connection parameters above.

Connection for use in R and Rmarkdown
...
> YOUR TURN: write queries that will 
>    1. to join the sites table to the traits table
...
Advantage for larger datasets: can do a lot of server-side summarization and some computing before pulling data into memory. 
